...
class GraphGUI(tk.Tk):
	...
	def update_graph(self, huh=None):
		...
		t_list = self.handler.get_dummy_graph_variables(self.user_id,
			method=self.method, oid=self.objective_id)
		self.a.clear()
		self.a.plot(range(len(graph_list)), graph_list)
		# self.a.plot(range(len(graph_list)), t_list)

		for b1, b2, c in zip(boundary_list[:-1], boundary_list[1:], color_list):
			self.a.broken_barh([(b1, b2-b1)],
				(.15*max(graph_list), .7*max(graph_list)), facecolors=c)
	...
	def move_user(self, incr):
		...
		self.user_id = self.handler.get_users()[idx%len(self.handler.get_users())]
		return self.user_id
			...
			for learn_obj in np.unique(self.handler.learn_obj_ids)[1:]:
				...
				for b1, b2, c in zip(boundary_list[:-1], boundary_list[1:],
					...
					a.broken_barh([(b1, b2 - b1)],
						(.15 * max(graph_list), .7 * max(graph_list)), facecolors=c)
				fname = 'student {} objective {}.png'.format(user, learn_obj)
...
class GraphPage(tk.Frame):
	...
	def __init__(self, parent, controller):
		...
		menu = ttk.OptionMenu(navigation_frame, self.stud_id_variable,
			stud_id_options[0], *stud_id_options, command=controller.upd_user)
		learning_goal_menu = ttk.OptionMenu(navigation_frame,
			self.learn_obj_id_variable, learn_obj_id_options[0],
			*learn_obj_id_options, command=controller.upd_learn_goal)
		button2 = ttk.Button(navigation_frame, text='>',
												 command=lambda: self.next_user())
		button1.grid(column=0, row=0)
		ttk.Label(navigation_frame, text="Pick a student:").grid(column=1, row=0)
		menu.grid(column=2, row=0)
		button2.grid(column=3, row=0)
		ttk.Label(navigation_frame, text="Pick the learning goal:").grid(column=4,
			row=0)
		learning_goal_menu.grid(column=5, row=0)
		navigation_frame.pack()

		method_frame = tk.Frame(self)
		ttk.Label(method_frame, text='Include attempts of exercise:').grid(row=0,
			column=0)
		...
		for i, pick in enumerate(self.methods):
			chk = ttk.Radiobutton(method_frame, text=pick, variable=self.method_var,
			value=i, command=self.change_method)
			chk.grid(row=0, column=i+1)
		ttk.Button(method_frame, text='Save graph',
			command=controller.save_graph).grid(row=0, column=len(self.methods)+1)
		ttk.Button(method_frame,
			text='Save all graphs', command=controller.save_all_graphs).grid(row=0,
			column=len(self.methods)+2)
		method_frame.pack()
